...
class groceries:
    ...
    def move(self, coord):
        self.rob.transform_tool(coord, 0.2, 0.2)
        time.sleep(1)
        self.transform_tool([0, 0, 0.1], 0.2, 0.2)

    def grab(self, typ):
        while self.rob.get_force() < 10:
            self.rob.translate_tool([0, 0, 0.01], 0.3, 0.3)
            time.sleep(0.05)
        self.robotiqgrip.close_grip()
        if typ == "apple":
            self.translate_tool(self.bin3, 0.3, 0.3)
            self.robotiqgrip.open_gripper()
        elif typ == "carrot":
            self.translate_tool(self.bin2, 0.3, 0.3)
            self.robotiqgrip.open_gripper()
        else:
            self.translate_tool(self.bin1, 0.3, 0.3)
            self.robotiqgrip.open_gripper()
        self.rob.close()